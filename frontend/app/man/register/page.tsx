 ...
 * Route: /man/register
 * Requires invitation token for manager registration
 */
function ManagerRegisterPageContent() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setInvitationError('Invitation token is required for manager registration. Please use the invitation link sent to you.')
      setValidatingInvitation(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.invitation_token) {
      setError('Invitation token is required for manager registration.')
      return
    }
    ...
    try {
      ...
      if (response.user && response.user.role !== 'manager') {
        setError('Registration failed. Please contact admin to create a manager account.')
        return
      }
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center text-sm text-muted-foreground">
          <p>
            ...
            <Link href="/man/login" className="text-primary hover:underline">
              Login
            </Link>
          </p>
          <p className="mt-2">
