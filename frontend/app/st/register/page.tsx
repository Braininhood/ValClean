...
function StaffRegisterPageContent() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setInvitationError('Invitation token is required for staff registration. Please use the invitation link sent to you.')
      setValidatingInvitation(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.invitation_token) {
      setError('Invitation token is required for staff registration.')
      return
    }
    ...
    try {
      ...
      if (response.user && response.user.role !== 'staff') {
        setError('Registration failed. Please contact admin to create a staff account.')
        return
      }
      
      // Redirect to staff dashboard
      router.push('/st/dashboard')
    } catch (err: any) {
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center text-sm text-muted-foreground">
          <p>
            ...
            <Link href="/st/login" className="text-primary hover:underline">
              Login
            </Link>
          </p>
          <p className="mt-2">
